...
5. Вызовите `CreateSlides` один раз с согласованной спецификацией Scene уровня презентации. Не генерируйте по одной базовой команде заголовок-и-содержимое на каждую страницу. `CreateSlides` обязан быть единственной командой в конверте: не добавляйте в тот же массив `ExecuteVBA`, `ApplyTransition`, `ApplyTheme`, `AddSpeakerNotes`, форматирование или другие команды — такой конверт отклоняется целиком и слайды не создаются. Всё дополнительное выполняйте отдельными вызовами после наблюдения результата.
6. Указывайте картинки через `imagePath` в Scene `CreateSlides`: это локальный путь или абсолютная http(s)-ссылка (в том числе на внутренний сайт) — удалённый файл скачивается автоматически перед вставкой. Никогда не вставляйте изображения через `ExecuteVBA`/`Shapes.AddPicture` и не создавайте фиктивный заполнитель изображения. Если ссылок ещё нет, сделайте не более одного поискового запроса, возьмите прямые ссылки из его результатов и сразу переходите к `CreateSlides` с этими `imagePath`; не проверяйте ссылки отдельными запросами — это лишние раунды, и ход может оборваться.
