...
Namespace Agent
    ...
    Public Class PromptManager
        ...
        Public Function BuildPlanningPrompt(session As AgentSession,
            ...
            sb.AppendLine("План должен покрывать все критерии успеха. Если требуется изображение, используй только зарегистрированную возможность, создающую реальные изображения; PowerPoint принимает в slides[].imagePath у CreateSlides существующий локальный путь либо абсолютную http(s)-ссылку (удалённая картинка скачивается перед вставкой), поэтому ссылку можно указывать прямо в плане. Не выдумывай локальные пути к файлам, которых нет. При отсутствии источника изображений верни capabilityGap; запрещено подменять изображение фигурой-заглушкой или умалчивать о нём и заявлять о выполнении.")
