...
| Статус | P0 закрыт: переведены промпты, tool/skill/JSON-schema описания, реестры, матчеры намерений, память, перевод и системные сообщения. Остались контент `StyleGuideManager` (справочные стандарты) и P1 UI |
