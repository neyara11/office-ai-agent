...
| Статус | P0 закрыт (промпты, tool/skill/JSON-schema описания, реестры, матчеры намерений, память, перевод, системные сообщения, справочные стандарты). P1 в основном закрыт: переведены лента (базовая + три хоста), диалоги, чат-панель WebView2, UI веб-краулера, пользовательские сообщения/промпты `BaseChatControl`/`Controls.Services`/хостов и классы ошибок; добавлена локальная справка-заглушка, внешние ссылки убраны. Все четыре проекта собираются. Осталось: контент полной справки (P1), layout-фиксы (P2), offline TLS и внешние панели (P3) |
